...
import com.microsoft.azure.servicebus.util.BackPortUtil;
...
public class TestCommons {
    ...
    public static void testSendSceduledMessageAndCancel(IMessageSender sender, String sessionId, IMessageReceiver receiver) throws InterruptedException, ServiceBusException {
        int secondsToWaitBeforeScheduling = 30;
        final String msgId1 = UUID.randomUUID().toString();
        ...
        Assert.assertTrue("Scheduled messages not received", BackPortUtil.removeIf(allReceivedMessages, new Predicate<IMessage>() {
            @Override
            public boolean test(IMessage msg) {
                return msg.getMessageId().equals(msgId1);
            }
        }));
        Assert.assertFalse("Cancelled scheduled messages also received", BackPortUtil.removeIf(allReceivedMessages, new Predicate<IMessage>() {
            @Override
            public boolean test(IMessage msg) {
                return msg.getMessageId().equals(msgId2);
            }
        }));
    }
        ...
        if (browsableSessions != null && browsableSessions.size() > 0) {
            ...
            for (IMessageSession browsableSession : browsableSessions) {
                CompletableFuture<Void> drainFuture = ClientFactory.acceptSessionFromEntityPathAsync
                        (TestUtils.getNamespaceEndpointURI(), receivePath, browsableSession.getSessionId(), TestUtils.getClientSettings(), ReceiveMode.RECEIVEANDDELETE).thenAcceptAsync(new Consumer<IMessageSession>() {
                    @Override
                    public void accept(IMessageSession session) {
                        try {
