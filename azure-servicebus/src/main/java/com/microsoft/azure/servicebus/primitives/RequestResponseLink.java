...
class RequestResponseLink extends ClientEntity {
    ...
    public static CompletableFuture<RequestResponseLink> createAsync(final MessagingFactory messagingFactory, String linkName, String linkPath, String sasTokenAudienceURI, MessagingEntityType entityType) {
        ...
        requestReponseLink.sendTokenAndSetRenewTimer(false).handleAsync(new BiFunction<Void, Throwable, Object>() {
            @Override
        ...
        CompletableFuture.allOf(requestReponseLink.amqpSender.openFuture, requestReponseLink.amqpReceiver.openFuture).handleAsync(new BiFunction<Void, Throwable, Object>() {
            @Override
    ...
    private CompletableFuture<Void> sendTokenAndSetRenewTimer(boolean retryOnFailure) {
        if (this.getIsClosingOrClosed() || this.sasTokenAudienceURI == null) {
            ...
            CompletableFuture<ScheduledFuture<?>> sendTokenFuture = this.underlyingFactory.sendSecurityTokenAndSetRenewTimer(this.sasTokenAudienceURI, retryOnFailure, new Runnable() {
                @Override
                public void run() {
                    RequestResponseLink.this.sendTokenAndSetRenewTimer(true);
                }
            });
            return sendTokenFuture.thenAccept(new Consumer<ScheduledFuture<?>>() {
                @Override
                public void accept(ScheduledFuture<?> f) {
                    RequestResponseLink.this.sasTokenRenewTimerFuture = f;
                }
            });
        }
    }
                    ...
                    if (exception != null && exception instanceof ServiceBusException && ((ServiceBusException) exception).getIsTransient()) {
                        ...
                        if (nextRetryInterval != null) {
                            Timer.schedule(new Runnable() {
                                @Override
                                public void run() {
                                    RequestResponseLink.this.ensureUniqueLinkRecreation();
                                }
    ...
    private void createInternalLinks() {
        ...
        if (this.sasTokenAudienceURI == null) {
            ...
            if (connection == null) {
                ...
                }, Duration.ofMillis(1000), TimerType.OneTimeRun);
                return;
            }
        }
    ...
    private void ensureUniqueLinkRecreation() {
        synchronized (this.recreateLinksLock) {
            if (!this.isRecreateLinksInProgress) {
                ...
                this.recreateInternalLinks().handleAsync(new BiFunction<Void, Throwable, Object>() {
                    @Override
                    public Object apply(Void v, Throwable recreationEx) {
                        if (recreationEx != null) {
                            TRACE_LOGGER.warn("Recreating internal links of reqestresponselink '{}' failed.", RequestResponseLink.this.linkPath, ExceptionUtil.extractAsyncCompletionCause(recreationEx));
                        }

                        synchronized (RequestResponseLink.this.recreateLinksLock) {
                            RequestResponseLink.this.isRecreateLinksInProgress = false;
                        }
    ...
    private CompletableFuture<Void> recreateInternalLinks() {
        ...
        this.sendTokenAndSetRenewTimer(false).handleAsync(new BiFunction<Void, Throwable, Object>() {
            ...
            public Object apply(Void v, Throwable sasTokenEx) {
                if (sasTokenEx != null) {
                    Throwable cause = ExceptionUtil.extractAsyncCompletionCause(sasTokenEx);
                    TRACE_LOGGER.error("Sending SAS Token failed. RequestResponseLink path:{}", RequestResponseLink.this.linkPath, cause);
                    recreateInternalLinksFuture.completeExceptionally(cause);
                    RequestResponseLink.this.completeAllPendingRequestsWithException(cause);
                } else {
                    try {
                        RequestResponseLink.this.underlyingFactory.scheduleOnReactorThread(new DispatchHandler() {
                            @Override
                            public void onEvent() {
        ...
        CompletableFuture.allOf(this.amqpSender.openFuture, this.amqpReceiver.openFuture).handleAsync(new BiFunction<Void, Throwable, Object>() {
            @Override
            public Object apply(Void v, Throwable ex) {
                if (ex == null) {
                    TRACE_LOGGER.info("Recreated internal links to {}", RequestResponseLink.this.linkPath);
                    RequestResponseLink.this.underlyingFactory.getRetryPolicy().resetRetryCount(RequestResponseLink.this.getClientId());
    ...
    private class InternalReceiver extends ClientEntity implements IAmqpReceiver {
        ...
        public void onReceiveComplete(Delivery delivery) {
            ...
            MessagingFactory.INTERNAL_THREAD_POOL.submit(new Runnable() {
                @Override
                public void run() {
                    ...
                    if (requestMessageId != null) {
                        ...
                        InternalReceiver.this.parent.completeRequestWithResponse(requestMessageId, finalResponseMessage);
                    } else {
