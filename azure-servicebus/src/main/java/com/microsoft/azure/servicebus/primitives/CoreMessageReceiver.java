...
import static com.microsoft.azure.servicebus.util.BackPortUtil.updateAndGet;
...
public class CoreMessageReceiver extends ClientEntity implements IAmqpReceiver, IErrorContextProvider {
    ...
    private CompletableFuture<CoreMessageReceiver> createLink() {
        ...
        this.sendTokenAndSetRenewTimer(false).handleAsync(new BiFunction<Void, Throwable, Object>() {
            @Override
            public Object apply(Void v, Throwable sasTokenEx) {
                if (sasTokenEx != null) {
                    Throwable cause = ExceptionUtil.extractAsyncCompletionCause(sasTokenEx);
                    TRACE_LOGGER.error("Sending SAS Token failed. ReceivePath:{}", CoreMessageReceiver.this.receivePath, cause);
                    CoreMessageReceiver.this.linkOpen.getWork().completeExceptionally(cause);
                } else {
                    try {
                        ...
                        CoreMessageReceiver.this.linkOpen.getWork().completeExceptionally(new ServiceBusException(false, "Failed to create Receiver, see cause for more details.", ioException));
                    }
                }
    ...
    private CompletableFuture<Void> createRequestResponseLinkAsync() {
        synchronized (this.requestResonseLinkCreationLock) {
            if (this.requestResponseLinkCreationFuture == null) {
                ...
                this.underlyingFactory.obtainRequestResponseLinkAsync(this.receivePath, this.entityType).handleAsync(new BiFunction<RequestResponseLink, Throwable, Object>() {
                    @Override
                    public Object apply(RequestResponseLink rrlink, Throwable ex) {
                        if (ex == null) {
                            CoreMessageReceiver.this.requestResponseLink = rrlink;
                            CoreMessageReceiver.this.requestResponseLinkCreationFuture.complete(null);
                        } else {
                            Throwable cause = ExceptionUtil.extractAsyncCompletionCause(ex);
                            CoreMessageReceiver.this.requestResponseLinkCreationFuture.completeExceptionally(cause);
                            // Set it to null so next call will retry rr link creation
                            synchronized (CoreMessageReceiver.this.requestResonseLinkCreationLock) {
                                CoreMessageReceiver.this.requestResponseLinkCreationFuture = null;
                            }
                        }
                        return null;
                    }
        ...
        synchronized (this.requestResonseLinkCreationLock) {
            if (this.requestResponseLinkCreationFuture != null) {
                this.requestResponseLinkCreationFuture.thenRun(new Runnable() {
                    @Override
                    public void run() {
                        CoreMessageReceiver.this.underlyingFactory.releaseRequestResponseLink(CoreMessageReceiver.this.receivePath);
                        CoreMessageReceiver.this.requestResponseLink = null;
                    }
                });
    ...
    CompletableFuture<Void> sendTokenAndSetRenewTimer(boolean retryOnFailure) {
        if (this.getIsClosingOrClosed()) {
            ...
            CompletableFuture<ScheduledFuture<?>> sendTokenFuture = this.underlyingFactory.sendSecurityTokenAndSetRenewTimer(this.sasTokenAudienceURI, retryOnFailure, new Runnable() {
                @Override
                public void run() {
                    CoreMessageReceiver.this.sendTokenAndSetRenewTimer(true);
                }
            });
            return sendTokenFuture.thenAccept(new Consumer<ScheduledFuture<?>>() {
                @Override
                public void accept(ScheduledFuture<?> f) {
                    CoreMessageReceiver.this.sasTokenRenewTimerFuture = f;
                }
            });
        }
    }
    ...
    public CompletableFuture<Collection<MessageWithDeliveryTag>> receiveAsync(final int maxMessageCount, Duration timeout) {
        ...
        this.ensureLinkIsOpen().thenRun(new Runnable() {
            @Override
            public void run() {
                CoreMessageReceiver.this.addCredit(receiveWorkItem);
            }
        });
        return onReceive;
    }
    ...
    public void onReceiveComplete(final Delivery delivery) {
        ...
        if (deliveryTag == null || deliveryTag.length == 0 || !this.tagsToDeliveriesMap.containsKey(deliveryTagAsString)) {
            ...
            if (remoteState instanceof Outcome) {
                ...
                final UpdateStateWorkItem matchingUpdateStateWorkItem = this.pendingUpdateStateRequests.get(deliveryTagAsString);
...
//						}
    ...
    private void reduceCreditForCompletedReceiveRequest(final int maxCreditCountOfReceiveRequest) {
        ...
        updateAndGet(creditNeededtoServePendingReceives, intUnaryOperator);
    }
    ...
    private CompletableFuture<Void> updateMessageStateAsync(byte[] deliveryTag, final Outcome outcome) {
        this.throwIfInUnusableState();
        final CompletableFuture<Void> completeMessageFuture = new CompletableFuture<Void>();
        ...
        final Delivery delivery = CoreMessageReceiver.this.tagsToDeliveriesMap.get(deliveryTagAsString);
        if (delivery == null) {
            ...
            CoreMessageReceiver.this.ensureLinkIsOpen().thenRun(new Runnable() {
                @Override
                public void run() {
                    try {
                        CoreMessageReceiver.this.underlyingFactory.scheduleOnReactorThread(new DispatchHandler() {
    ...
    private synchronized CompletableFuture<Void> ensureLinkIsOpen() {
        ...
        if (!(this.receiveLink.getLocalState() == EndpointState.ACTIVE && this.receiveLink.getRemoteState() == EndpointState.ACTIVE)) {
            if (this.receiveLinkReopenFuture == null || this.receiveLinkReopenFuture.isDone()) {
                ...
                this.sendTokenAndSetRenewTimer(false).handleAsync(new BiFunction<Void, Throwable, Object>() {
                    @Override
                    public Object apply(Void v, Throwable sendTokenEx) {
                        if (sendTokenEx != null) {
                            Throwable cause = ExceptionUtil.extractAsyncCompletionCause(sendTokenEx);
                            TRACE_LOGGER.error("Sending SAS Token to '{}' failed.", CoreMessageReceiver.this.receivePath, cause);
                            CoreMessageReceiver.this.receiveLinkReopenFuture.completeExceptionally(sendTokenEx);
                            CoreMessageReceiver.this.clearAllPendingWorkItems(sendTokenEx);
                        } else {
                            try {
                                CoreMessageReceiver.this.underlyingFactory.scheduleOnReactorThread(new DispatchHandler() {
                                    @Override
                                ...
                                CoreMessageReceiver.this.receiveLinkReopenFuture.completeExceptionally(ioEx);
                            }
                        }
                        return null;
                    }
            ...
            this.receiveLinkReopenFuture.handleAsync(new BiFunction<Void, Throwable, Object>() {
                                                         @Override
                                                         public Object apply(Void v, Throwable ex) {
                                                             if (ex == null) {
                                                                 CoreMessageReceiver.this.ensureLinkReopenFutureToWaitOn.complete(null);
                                                             } else {
                                                                 if (ex instanceof ServiceBusException && ((ServiceBusException) ex).getIsTransient()) {
                                                                     if (CoreMessageReceiver.this.shouldRetryLinkReopenOnTransientFailure) {
                                                                         // Retry link creation
                                                                         CoreMessageReceiver.this.shouldRetryLinkReopenOnTransientFailure = false;
                                                                         CoreMessageReceiver.this.ensureLinkIsOpen();
                                                                     } else {
                                                                         CoreMessageReceiver.this.ensureLinkReopenFutureToWaitOn.completeExceptionally(ex);
                                                                     }
                                                                 } else {
                                                                     CoreMessageReceiver.this.ensureLinkReopenFutureToWaitOn.completeExceptionally(ex);
                                                                 }
    ...
    public CompletableFuture<Collection<Instant>> renewMessageLocksAsync(final UUID[] lockTokens) {
        this.throwIfInUnusableState();
        ...
        return this.createRequestResponseLinkAsync().thenComposeAsync(new Function<Void, CompletionStage<Collection<Instant>>>() {
            @Override
            public CompletionStage<Collection<Instant>> apply(Void v) {
                HashMap requestBodyMap = new HashMap();
                requestBodyMap.put(ClientConstants.REQUEST_RESPONSE_LOCKTOKENS, lockTokens);
                if (CoreMessageReceiver.this.isSessionReceiver) {
                    requestBodyMap.put(ClientConstants.REQUEST_RESPONSE_SESSIONID, CoreMessageReceiver.this.getSessionId());
                }

                Message requestMessage = RequestResponseUtils.createRequestMessageFromPropertyBag(ClientConstants.REQUEST_RESPONSE_RENEWLOCK_OPERATION, requestBodyMap, Util.adjustServerTimeout(CoreMessageReceiver.this.operationTimeout), CoreMessageReceiver.this.receiveLink.getName());
                CompletableFuture<Message> responseFuture = CoreMessageReceiver.this.requestResponseLink.requestAysnc(requestMessage, CoreMessageReceiver.this.operationTimeout);
                return responseFuture.thenComposeAsync(new Function<Message, CompletionStage<Collection<Instant>>>() {
                    @Override
                    public CompletionStage<Collection<Instant>> apply(Message responseMessage) {
                        ...
                        if (statusCode == ClientConstants.REQUEST_RESPONSE_OK_STATUS_CODE) {
                            ...
                            List<Instant> ret = StreamSupport.stream(Arrays.asList(expirations)).map(new Function<Date, Instant>() {
                                @Override
                                public Instant apply(Date d) {
                                    return Instant.ofEpochMilli(d.getTime());
                                }
                            }).collect(java8.util.stream.Collectors.<Instant>toList());
                            returningFuture.complete(ret);
                        } else {
                            // error response
                            Exception failureException = RequestResponseUtils.genereateExceptionFromResponse(responseMessage);
                            TRACE_LOGGER.error("Renewing message locks for lock tokens '{}' on entity '{}' failed", Arrays.toString(lockTokens), CoreMessageReceiver.this.receivePath, failureException);
    ...
    public CompletableFuture<Collection<MessageWithLockToken>> receiveDeferredMessageBatchAsync(final Long[] sequenceNumbers) {
        this.throwIfInUnusableState();
        if (TRACE_LOGGER.isDebugEnabled()) {
        ...
        return this.createRequestResponseLinkAsync().thenComposeAsync(new Function<Void, CompletionStage<Collection<MessageWithLockToken>>>() {
            @Override
            public CompletionStage<Collection<MessageWithLockToken>> apply(Void v) {
                HashMap requestBodyMap = new HashMap();
                requestBodyMap.put(ClientConstants.REQUEST_RESPONSE_SEQUENCE_NUMBERS, sequenceNumbers);
                requestBodyMap.put(ClientConstants.REQUEST_RESPONSE_RECEIVER_SETTLE_MODE, UnsignedInteger.valueOf(CoreMessageReceiver.this.settleModePair.getReceiverSettleMode() == ReceiverSettleMode.FIRST ? 0 : 1));
                if (CoreMessageReceiver.this.isSessionReceiver) {
                    requestBodyMap.put(ClientConstants.REQUEST_RESPONSE_SESSIONID, CoreMessageReceiver.this.getSessionId());
                }

                Message requestMessage = RequestResponseUtils.createRequestMessageFromPropertyBag(ClientConstants.REQUEST_RESPONSE_RECEIVE_BY_SEQUENCE_NUMBER, requestBodyMap, Util.adjustServerTimeout(CoreMessageReceiver.this.operationTimeout), CoreMessageReceiver.this.receiveLink.getName());
                CompletableFuture<Message> responseFuture = CoreMessageReceiver.this.requestResponseLink.requestAysnc(requestMessage, CoreMessageReceiver.this.operationTimeout);
                return responseFuture.thenComposeAsync(new Function<Message, CompletionStage<Collection<MessageWithLockToken>>>() {
                    @Override
                    public CompletionStage<Collection<MessageWithLockToken>> apply(Message responseMessage) {
                        CompletableFuture<Collection<MessageWithLockToken>> returningFuture = new CompletableFuture<Collection<MessageWithLockToken>>();
                        int statusCode = RequestResponseUtils.getResponseStatusCode(responseMessage);
                        if (statusCode == ClientConstants.REQUEST_RESPONSE_OK_STATUS_CODE) {
                            if (TRACE_LOGGER.isDebugEnabled()) {
                                TRACE_LOGGER.debug("Received messges for sequence numbers '{}' from entity '{}', sesion '{}'", Arrays.toString(sequenceNumbers), CoreMessageReceiver.this.receivePath, CoreMessageReceiver.this.isSessionReceiver ? CoreMessageReceiver.this.getSessionId() : "");
                            }
                            ...
                            // error response
                            Exception failureException = RequestResponseUtils.genereateExceptionFromResponse(responseMessage);
                            TRACE_LOGGER.error("Receiving messages by sequence numbers '{}' from entity '{}' failed", Arrays.toString(sequenceNumbers), CoreMessageReceiver.this.receivePath, failureException);
                            returningFuture.completeExceptionally(failureException);
                        }
                        return returningFuture;
                    }
    ...
    public CompletableFuture<Void> updateDispositionAsync(final UUID[] lockTokens, final String dispositionStatus, final String deadLetterReason, final String deadLetterErrorDescription, final Map<String, Object> propertiesToModify) {
        this.throwIfInUnusableState();
        if (TRACE_LOGGER.isDebugEnabled()) {
        ...
        return this.createRequestResponseLinkAsync().thenComposeAsync(new Function<Void, CompletionStage<Void>>() {
            @Override
            public CompletionStage<Void> apply(Void v) {
                ...
                Message requestMessage = RequestResponseUtils.createRequestMessageFromPropertyBag(ClientConstants.REQUEST_RESPONSE_UPDATE_DISPOSTION_OPERATION, requestBodyMap, Util.adjustServerTimeout(CoreMessageReceiver.this.operationTimeout), CoreMessageReceiver.this.receiveLink.getName());
                CompletableFuture<Message> responseFuture = CoreMessageReceiver.this.requestResponseLink.requestAysnc(requestMessage, CoreMessageReceiver.this.operationTimeout);
                return responseFuture.thenComposeAsync(new Function<Message, CompletionStage<Void>>() {
                    ...
                    public CompletionStage<Void> apply(Message responseMessage) {
                        ...
                        if (statusCode == ClientConstants.REQUEST_RESPONSE_OK_STATUS_CODE) {
                            if (TRACE_LOGGER.isDebugEnabled()) {
                                TRACE_LOGGER.debug("Update disposition of deliveries '{}' to '{}' on entity '{}', sesion '{}' succeeded.", Arrays.toString(lockTokens), dispositionStatus, CoreMessageReceiver.this.receivePath, CoreMessageReceiver.this.isSessionReceiver ? CoreMessageReceiver.this.getSessionId() : "");
                            }
                            returningFuture.complete(null);
                        } else {
                            // error response
                            Exception failureException = RequestResponseUtils.genereateExceptionFromResponse(responseMessage);
                            TRACE_LOGGER.error("Update disposition on entity '{}' failed", CoreMessageReceiver.this.receivePath, failureException);
                            returningFuture.completeExceptionally(failureException);
                        }
                        return returningFuture;
                    }
    ...
    public CompletableFuture<Void> renewSessionLocksAsync() {
        ...
        return this.createRequestResponseLinkAsync().thenComposeAsync(new Function<Void, CompletionStage<Void>>() {
            ...
            public CompletionStage<Void> apply(Void v) {
                ...
                Message requestMessage = RequestResponseUtils.createRequestMessageFromPropertyBag(ClientConstants.REQUEST_RESPONSE_RENEW_SESSIONLOCK_OPERATION, requestBodyMap, Util.adjustServerTimeout(CoreMessageReceiver.this.operationTimeout), CoreMessageReceiver.this.receiveLink.getName());
                CompletableFuture<Message> responseFuture = CoreMessageReceiver.this.requestResponseLink.requestAysnc(requestMessage, CoreMessageReceiver.this.operationTimeout);
                return responseFuture.thenComposeAsync(new Function<Message, CompletionStage<Void>>() {
                    @Override
                    public CompletionStage<Void> apply(Message responseMessage) {
                        ...
                        if (statusCode == ClientConstants.REQUEST_RESPONSE_OK_STATUS_CODE) {
                            ...
                            CoreMessageReceiver.this.sessionLockedUntilUtc = Instant.ofEpochMilli(expiration.getTime());
                            TRACE_LOGGER.debug("Session lock on entity '{}' of sesion '{}' renewed until '{}'", CoreMessageReceiver.this.receivePath, CoreMessageReceiver.this.getSessionId(), CoreMessageReceiver.this.sessionLockedUntilUtc);
                            returningFuture.complete(null);
                        } else {
                            // error response
                            Exception failureException = RequestResponseUtils.genereateExceptionFromResponse(responseMessage);
                            TRACE_LOGGER.error("Renewing session lock on entity '{}' of sesion '{}' failed", CoreMessageReceiver.this.receivePath, CoreMessageReceiver.this.getSessionId(), failureException);
                            returningFuture.completeExceptionally(failureException);
                        }
                        return returningFuture;
                    }
    ...
    public CompletableFuture<byte[]> getSessionStateAsync() {
        ...
        return this.createRequestResponseLinkAsync().thenComposeAsync(new Function<Void, CompletionStage<byte[]>>() {
            @Override
            public CompletionStage<byte[]> apply(Void v) {
                HashMap requestBodyMap = new HashMap();
                requestBodyMap.put(ClientConstants.REQUEST_RESPONSE_SESSIONID, CoreMessageReceiver.this.getSessionId());

                Message requestMessage = RequestResponseUtils.createRequestMessageFromPropertyBag(ClientConstants.REQUEST_RESPONSE_GET_SESSION_STATE_OPERATION, requestBodyMap, Util.adjustServerTimeout(CoreMessageReceiver.this.operationTimeout), CoreMessageReceiver.this.receiveLink.getName());
                CompletableFuture<Message> responseFuture = CoreMessageReceiver.this.requestResponseLink.requestAysnc(requestMessage, CoreMessageReceiver.this.operationTimeout);
                return responseFuture.thenComposeAsync(new Function<Message, CompletionStage<byte[]>>() {
                    @Override
                    public CompletionStage<byte[]> apply(Message responseMessage) {
                        CompletableFuture<byte[]> returningFuture = new CompletableFuture<byte[]>();
                        int statusCode = RequestResponseUtils.getResponseStatusCode(responseMessage);
                        if (statusCode == ClientConstants.REQUEST_RESPONSE_OK_STATUS_CODE) {
                            TRACE_LOGGER.debug("Got session state of sesion '{}' from entity '{}'", CoreMessageReceiver.this.getSessionId(), CoreMessageReceiver.this.receivePath);
                            ...
                            // error response
                            Exception failureException = RequestResponseUtils.genereateExceptionFromResponse(responseMessage);
                            TRACE_LOGGER.error("Getting session state of sesion '{}' from entity '{}' failed", CoreMessageReceiver.this.getSessionId(), CoreMessageReceiver.this.receivePath, failureException);
                            returningFuture.completeExceptionally(failureException);
                        }
                        return returningFuture;
                    }
    ...
    public CompletableFuture<Void> setSessionStateAsync(final byte[] sessionState) {
        this.throwIfInUnusableState();
        ...
        return this.createRequestResponseLinkAsync().thenComposeAsync(new Function<Void, CompletionStage<Void>>() {
            ...
            public CompletionStage<Void> apply(Void v) {
                ...
                Message requestMessage = RequestResponseUtils.createRequestMessageFromPropertyBag(ClientConstants.REQUEST_RESPONSE_SET_SESSION_STATE_OPERATION, requestBodyMap, Util.adjustServerTimeout(CoreMessageReceiver.this.operationTimeout), CoreMessageReceiver.this.receiveLink.getName());
                CompletableFuture<Message> responseFuture = CoreMessageReceiver.this.requestResponseLink.requestAysnc(requestMessage, CoreMessageReceiver.this.operationTimeout);
                return responseFuture.thenComposeAsync(new Function<Message, CompletionStage<Void>>() {
                    @Override
                    public CompletionStage<Void> apply(Message responseMessage) {
                        ...
                        int statusCode = RequestResponseUtils.getResponseStatusCode(responseMessage);
                        if (statusCode == ClientConstants.REQUEST_RESPONSE_OK_STATUS_CODE) {
                            TRACE_LOGGER.debug("Setting session state of sesion '{}' on entity '{}' succeeded", CoreMessageReceiver.this.getSessionId(), CoreMessageReceiver.this.receivePath);
                            returningFuture.complete(null);
                        } else {
                            // error response
                            Exception failureException = RequestResponseUtils.genereateExceptionFromResponse(responseMessage);
                            TRACE_LOGGER.error("Setting session state of sesion '{}' on entity '{}' failed", CoreMessageReceiver.this.getSessionId(), CoreMessageReceiver.this.receivePath, failureException);
                            returningFuture.completeExceptionally(failureException);
                        }
                        return returningFuture;
                    }
    ...
    public CompletableFuture<Collection<Message>> peekMessagesAsync(final long fromSequenceNumber, final int messageCount, final String sessionId) {
        this.throwIfInUnusableState();
        return this.createRequestResponseLinkAsync().thenComposeAsync(new Function<Void, CompletionStage<Collection<Message>>>() {
            @Override
            public CompletionStage<Collection<Message>> apply(Void v) {
                return CommonRequestResponseOperations.peekMessagesAsync(CoreMessageReceiver.this.requestResponseLink, CoreMessageReceiver.this.operationTimeout, fromSequenceNumber, messageCount, sessionId, CoreMessageReceiver.this.receiveLink.getName());
            }
        }, MessagingFactory.INTERNAL_THREAD_POOL);
    }
}
