...
public class CoreMessageSender extends ClientEntity implements IAmqpSender, IErrorContextProvider {
    ...
    public static CompletableFuture<CoreMessageSender> create(
        ...
        msgSender.sendTokenAndSetRenewTimer(false).handleAsync(new BiFunction<Void, Throwable, Object>() {
            @Override
    ...
    private CompletableFuture<Void> createRequestResponseLink() {
        synchronized (this.requestResonseLinkCreationLock) {
            if (this.requestResponseLinkCreationFuture == null) {
                ...
                this.underlyingFactory.obtainRequestResponseLinkAsync(this.sendPath, this.entityType).handleAsync(new BiFunction<RequestResponseLink, Throwable, Object>() {
                    @Override
                    public Object apply(RequestResponseLink rrlink, Throwable ex) {
                        if (ex == null) {
                            CoreMessageSender.this.requestResponseLink = rrlink;
                            CoreMessageSender.this.requestResponseLinkCreationFuture.complete(null);
                        } else {
                            Throwable cause = ExceptionUtil.extractAsyncCompletionCause(ex);
                            CoreMessageSender.this.requestResponseLinkCreationFuture.completeExceptionally(cause);
                            // Set it to null so next call will retry rr link creation
                            synchronized (CoreMessageSender.this.requestResonseLinkCreationLock) {
                                CoreMessageSender.this.requestResponseLinkCreationFuture = null;
                            }
                        }
                        return null;
                    }
        ...
        synchronized (this.requestResonseLinkCreationLock) {
            if (this.requestResponseLinkCreationFuture != null) {
                this.requestResponseLinkCreationFuture.thenRun(new Runnable() {
                    @Override
                    public void run() {
                        CoreMessageSender.this.underlyingFactory.releaseRequestResponseLink(CoreMessageSender.this.sendPath);
                        CoreMessageSender.this.requestResponseLink = null;
                    }
                });
    ...
    private void scheduleSendTimeout(final SendWorkItem sendWorkItem) {
                    ...
                    if (tracker != null) {
                        ...
                        if (nextRetryInterval != null) {
                            ...
                            Timer.schedule(new Runnable() {
                                @Override
                                public void run() {
                                    CoreMessageSender.this.ensureLinkIsOpen();
                                }
    ...
    CompletableFuture<Void> sendTokenAndSetRenewTimer(boolean retryOnFailure) {
        if (this.getIsClosingOrClosed()) {
            ...
            CompletableFuture<ScheduledFuture<?>> sendTokenFuture = this.underlyingFactory.sendSecurityTokenAndSetRenewTimer(this.sasTokenAudienceURI, retryOnFailure, new Runnable() {
                @Override
                public void run() {
                    CoreMessageSender.this.sendTokenAndSetRenewTimer(true);
                }
            });
            return sendTokenFuture.thenAccept(new Consumer<ScheduledFuture<?>>() {
                @Override
                public void accept(ScheduledFuture<?> f) {
                    CoreMessageSender.this.sasTokenRenewTimerFuture = f;
                }
            });
        }
    }
                ...
                this.sendTokenAndSetRenewTimer(false).handleAsync(new BiFunction<Void, Throwable, Object>() {
                    ...
                    public Object apply(Void v, Throwable sendTokenEx) {
                        if (sendTokenEx != null) {
                            ...
                            TRACE_LOGGER.error("Sending SAS Token to '{}' failed.", CoreMessageSender.this.sendPath, cause);
                            CoreMessageSender.this.sendLinkReopenFuture.completeExceptionally(sendTokenEx);
                            CoreMessageSender.this.clearAllPendingSendsWithException(sendTokenEx);
                        } else {
                            try {
                                CoreMessageSender.this.underlyingFactory.scheduleOnReactorThread(new DispatchHandler() {
                                    @Override
                                    public void onEvent() {
                                ...
                                CoreMessageSender.this.sendLinkReopenFuture.completeExceptionally(ioEx);
                            }
                        }
                        return null;
                    }
    ...
    public CompletableFuture<long[]> scheduleMessageAsync(final Message[] messages, final Duration timeout) {
        TRACE_LOGGER.debug("Sending '{}' scheduled message(s) to '{}'", messages.length, this.sendPath);
        return this.createRequestResponseLink().thenComposeAsync(new Function<Void, CompletionStage<long[]>>() {
            @Override
            public CompletionStage<long[]> apply(Void v) {
                ...
                for (Message message : messages) {
                    ...
                    try {
                        encodedPair = Util.encodeMessageToOptimalSizeArray(message, CoreMessageSender.this.maxMessageSize);
                    } catch (PayloadSizeExceededException exception) {
                        TRACE_LOGGER.error("Payload size of message exceeded limit", exception);
                ...
                Message requestMessage = RequestResponseUtils.createRequestMessageFromPropertyBag(ClientConstants.REQUEST_RESPONSE_SCHEDULE_MESSAGE_OPERATION, requestBodyMap, Util.adjustServerTimeout(timeout), CoreMessageSender.this.sendLink.getName());
                CompletableFuture<Message> responseFuture = CoreMessageSender.this.requestResponseLink.requestAysnc(requestMessage, timeout);
                return responseFuture.thenComposeAsync(new Function<Message, CompletionStage<long[]>>() {
                    @Override
                    public CompletionStage<long[]> apply(Message responseMessage) {
                        ...
                        if (statusCode == ClientConstants.REQUEST_RESPONSE_OK_STATUS_CODE) {
                            ...
                            TRACE_LOGGER.error("Sending scheduled messages to '{}' failed.", CoreMessageSender.this.sendPath, scheduleException);
    ...
    public CompletableFuture<Void> cancelScheduledMessageAsync(final Long[] sequenceNumbers, final Duration timeout) {
        if (TRACE_LOGGER.isDebugEnabled()) {
        ...
        return this.createRequestResponseLink().thenComposeAsync(new Function<Void, CompletionStage<Void>>() {
            ...
            public CompletionStage<Void> apply(Void v) {
                ...
                Message requestMessage = RequestResponseUtils.createRequestMessageFromPropertyBag(ClientConstants.REQUEST_RESPONSE_CANCEL_CHEDULE_MESSAGE_OPERATION, requestBodyMap, Util.adjustServerTimeout(timeout), CoreMessageSender.this.sendLink.getName());
                CompletableFuture<Message> responseFuture = CoreMessageSender.this.requestResponseLink.requestAysnc(requestMessage, timeout);
                return responseFuture.thenComposeAsync(new Function<Message, CompletionStage<Void>>() {
                    ...
                    public CompletionStage<Void> apply(Message responseMessage) {
                        ...
                        int statusCode = RequestResponseUtils.getResponseStatusCode(responseMessage);
                        if (statusCode == ClientConstants.REQUEST_RESPONSE_OK_STATUS_CODE) {
                            TRACE_LOGGER.debug("Cancelled scheduled messages in '{}'", CoreMessageSender.this.sendPath);
                            returningFuture.complete(null);
                        } else {
                            // error response
                            Exception failureException = RequestResponseUtils.genereateExceptionFromResponse(responseMessage);
                            TRACE_LOGGER.error("Cancelling scheduled messages in '{}' failed.", CoreMessageSender.this.sendPath, failureException);
    ...
    public CompletableFuture<Collection<Message>> peekMessagesAsync(final long fromSequenceNumber, final int messageCount) {
        TRACE_LOGGER.debug("Peeking '{}' messages in '{}' from sequence number '{}'", messageCount, this.sendPath, fromSequenceNumber);
        return this.createRequestResponseLink().thenComposeAsync(new Function<Void, CompletionStage<Collection<Message>>>() {
            @Override
            public CompletionStage<Collection<Message>> apply(Void v) {
                return CommonRequestResponseOperations.peekMessagesAsync(CoreMessageSender.this.requestResponseLink, CoreMessageSender.this.operationTimeout, fromSequenceNumber, messageCount, null, CoreMessageSender.this.sendLink.getName());
            }
        }, MessagingFactory.INTERNAL_THREAD_POOL);
    }
}
