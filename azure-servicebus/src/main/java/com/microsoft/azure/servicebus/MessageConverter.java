...
class MessageConverter {
    public static org.apache.qpid.proton.message.Message convertBrokeredMessageToAmqpMessage(Message brokeredMessage) {
        ...
        if (brokeredMessage.getScheduledEnqueueTimeUtc() != null) {
            messageAnnotationsMap.put(Symbol.valueOf(ClientConstants.SCHEDULEDENQUEUETIMENAME), new Date(brokeredMessage.getScheduledEnqueueTimeUtc().toEpochMilli()));
        }
                ...
                for (Map.Entry<Symbol, Object> entry : messageAnnotationsMap.entrySet()) {
                    ...
                    switch (entryName) {
                        case ClientConstants.ENQUEUEDTIMEUTCNAME:
                            brokeredMessage.setEnqueuedTimeUtc(Instant.ofEpochMilli(date.getTime()));
                            break;
                        case ClientConstants.SCHEDULEDENQUEUETIMENAME:
                            brokeredMessage.setScheduledEnqueueTimeUtc(Instant.ofEpochMilli(date.getTime()));
                            break;
                        ...
                        case ClientConstants.LOCKEDUNTILNAME:
                            brokeredMessage.setLockedUntilUtc(Instant.ofEpochMilli(date.getTime()));
                            break;
