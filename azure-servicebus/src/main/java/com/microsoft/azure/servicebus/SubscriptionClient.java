...
public final class SubscriptionClient extends InitializableEntity implements ISubscriptionClient {
    ...
    public SubscriptionClient(ConnectionStringBuilder amqpConnectionStringBuilder, ReceiveMode receiveMode) throws InterruptedException, ServiceBusException {
        ...
        Utils.completeFuture(factoryFuture.thenComposeAsync(new Function<MessagingFactory, CompletableFuture<Void>>() {
            ...
            public CompletableFuture<Void> apply(MessagingFactory f) {
                return SubscriptionClient.this.createPumpAndBrowserAsync(f);
            }
    ...
    private CompletableFuture<Void> createPumpAndBrowserAsync(final MessagingFactory factory) {
        this.factory = factory;
        CompletableFuture<Void> postSessionBrowserFuture = MiscRequestResponseOperationHandler.create(factory, this.subscriptionPath, MessagingEntityType.SUBSCRIPTION).thenAcceptAsync(new Consumer<MiscRequestResponseOperationHandler>() {
            @Override
            public void accept(MiscRequestResponseOperationHandler msoh) {
                SubscriptionClient.this.miscRequestResponseHandler = msoh;
                SubscriptionClient.this.sessionBrowser = new SessionBrowser(factory, SubscriptionClient.this.subscriptionPath, MessagingEntityType.SUBSCRIPTION, msoh);
            }
        }, MessagingFactory.INTERNAL_THREAD_POOL);
    ...
    protected CompletableFuture<Void> onClose() {
        return this.messageAndSessionPump.closeAsync().thenCompose(new Function<Void, CompletionStage<Void>>() {
            ...
            public CompletionStage<Void> apply(Void v) {
                return SubscriptionClient.this.miscRequestResponseHandler.closeAsync().thenCompose(new Function<Void, CompletableFuture<Void>>() {
                    @Override
                    public CompletableFuture<Void> apply(Void w) {
                        return SubscriptionClient.this.factory.closeAsync();
                    }
                });
            }
        });
